...
class Trainer:
    ...
    def __init__(
            ...
            verbose="INFO",
            start_stage: int = 1, # for train from scratch
            restart_stage: Optional[int] = None, # for restart only
            **kwargs,
    ):
        ...
        self.stage = restart_stage if restart_stage is not None else start_stage
    ...
    def as_dict(
        ...
        if training_progress:
            dictionary["progress"] = {}
            for key in ["iepoch", "best_epoch", "stage"]:
    ...
    def from_dict(cls, dictionary, append: Optional[bool] = None):
        ...
        # update stage
        dictionary["restart_stage"] = stage
    ...
    def init(self):
        ...
        # set model stage
        core_func = self._find_core_module()
        if core_func is not None:
            core_func.ReciprocalNN.stage.fill_(self.stage)
            core_func.charge_mlp.stage.fill_(self.stage)
        # set model weight requires_grad states
        self._set_model_grad()
    ...
    def _find_core_module(self):
        """
        Recursively search for the core module containing 'ReciprocalNN'.
        This handles arbitrary nesting of wrapper layers (Rescale, Stress, etc.).
        """
        current_module = self.model

        while True:
            if hasattr(current_module, "func") and hasattr(current_module.func, "ReciprocalNN"):
                return current_module.func

            if hasattr(current_module, "model"):
                if hasattr(current_module.model, "func") and hasattr(current_module.model.func, "ReciprocalNN"):
                    return current_module.model.func
                current_module = current_module.model
            else:
                return None

    def _set_model_grad(self):
        '''
        Set model requires_grad status according to the training stage.
        '''
        # all parameters are default to requires_grad=True
        ...
        if core_func is not None:
            if self.stage == 1:
                # stage 1: train short-range part, freeze long-range part
                # forward process: skip long-range part
                for param in core_func.ReciprocalNN.parameters():
                    param.requires_grad = False
                for param in core_func.charge_mlp.parameters():
                    param.requires_grad = False
            elif self.stage == 2:
                # stage 2: freeze short-range part, train long-range part
                # forward process: both parts
                for param in core_func.allegro.parameters():
                    param.requires_grad = False
            # stage 3: train both parts
            # forward process: both parts
    ...
    def batch_step(self, data, validation=False):
        ...
        weighted_loss = "loss_weight" in data.keys()
        ...
        if not validation:
            ...
            loss, loss_contrib = self.loss(pred=out, ref=data_for_loss, weighted_loss=weighted_loss)
        ...
        with torch.no_grad():
            if validation:
                ...
                loss, loss_contrib = self.loss(
                    pred=normalized_units_out, ref=data_for_loss, weighted_loss=weighted_loss
                )
    ...
    def _switch_stage(self):
        '''
        Since stage change only occurs in model with both short-range parts and long-range parts,
        we can skip hasattr(core_func, "ReciprocalNN").
        '''
        # set stages
        new_stage = self.stage + 1

        # Reset best_metrics when moving from stage 1 to 2
        # Stage 1 (short-range only) metrics are not comparable to Stage 2 (total charges loss enabled)
        if self.stage == 1:
            self.logger.info("Resetting best_metrics for new stage (introducing total charge loss).")
            self.best_metrics = float("inf")

        # reset trainer class
        self.logger.info(f"Stage {self.stage} -> Stage {new_stage}")
    ...
    def end_of_batch_log(self, batch_type: str):
        ...
        log_str = f"Stage {self.stage}   {self.iepoch+1:5d} {self.ibatch+1:5d}"

        header = "epoch, batch"
        log_header = f"Stage {self.stage} # Epoch batch"
    ...
    def set_dataset(
        ...
        dl_kwargs = dict(
            ...
            # prefetch data to optimize dataload
